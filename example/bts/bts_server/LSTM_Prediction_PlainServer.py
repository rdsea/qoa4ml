...
class LSTM_Prediction_PlainServer(object):
    def __init__(self, configuration,base_dir):
        ...
        self.model = ML_Loader(self.model_info,base_dir)
...
#for storing service
service_dict = {}
# assume it from the configuraton
...
def create_service():
    ...
    if req_data["model"] != None:
        ...
        if model["name"] == "LSTM":
            new_service = LSTM_Prediction_PlainServer(req_data,base_dir)
            ...
            if req_data["model"] != None:
                ...
                if (model_name not in service_dict.keys()):
                    if req_data["model"]["name"] == "LSTM":
                        req_service = LSTM_Prediction_PlainServer(req_data,base_dir)
